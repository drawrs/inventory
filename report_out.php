...
else 
{
	...
	if ($module == 'out' && $act == 'search')
	{
		...
		{
			$querySales = "SELECT * FROM as_sales_transactions WHERE invoiceDate BETWEEN '$startDate' AND '$endDate' ORDER BY invoiceNo DESC LIMIT $position,$limit";
		}
		else
		{
			$querySales = "SELECT * FROM as_sales_transactions ORDER BY invoiceNo DESC LIMIT $position,$limit";
		}
