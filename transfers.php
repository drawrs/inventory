...
else 
{
	...
	if ($module == 'transfer' && $act == 'delete')
	{
		...
		{
			$qty = $dataTrf['qty'];
			$queryUpdateMin = "UPDATE as_stock_products SET stock = stock-$qty WHERE factoryID = '$dataTrf[transferTo]' AND productID = '$dataTrf[productID]'";
			$sqlUpdateMin = mysqli_query($connect, $queryUpdateMin);
			
			if ($sqlUpdateMin)
			{
				$queryUpdatePlus = "UPDATE as_stock_products SET stock = stock+$qty WHERE factoryID = '$dataTrf[transferFrom]' AND productID = '$dataTrf[productID]'";
	...
	elseif ($module == 'transfer' && $act == 'detailtrf')
	{
		...
		$smarty->assign("transferTo", $dataTo['factoryName']);
	...
	elseif ($module == 'transfer' && $act == 'input')
	{
		...
		while ($dataTransfer = mysqli_fetch_array($sqlTransfer))
		{
			...
			if ($save)
			{
				...
				$queryStockMin = "UPDATE as_stock_products SET stock = stock-$qty WHERE productID = '$dataTransfer[productID]' AND factoryID = '$transferFrom'";
				$save1 = mysqli_query($connect, $queryStockMin);
				
				if ($save1)
				{
					$queryStockPlus = "UPDATE as_stock_products SET stock = stock+$qty WHERE productID = '$dataTransfer[productID]' AND factoryID = '$transferTo'";
	...
	elseif ($module == 'transfer' && $act == 'add')
	{
		...
		$queryFatory = "SELECT * FROM as_factories WHERE status = 'Y' ORDER BY factoryName ASC";
	...
	else
	{
		...
		$smarty->assign("breadcumbTitleSmall", "Halaman untuk melakukan transaksi atau mutasi transfer gudang.");
		$smarty->assign("breadcumbMenuName", "Transfer Gudang");
		$smarty->assign("breadcumbMenuSubName", "Transfer Gudang");
	}
