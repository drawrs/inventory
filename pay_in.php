...
else 
{
	...
	elseif ($module == 'payin' && $act == 'input')
	{
		...
		$sq1 = "SELECT * FROM as_buy_transactions WHERE invoiceNo = '$invoiceNo'";
		//print_r($sq1);
		//print_r($invoiceNo);
		$sql2 = $conn->query($sq1);
		// datapkan no spbNo
			$spbNoX = "";
		while ($sq3 = $sql2->fetch_assoc()) {
			//
			//print_r($sq3);
			// datapkan no spbNo
			$spbNoX = $sq3['spbNo'];
		...
		//echo "=====================================================";
		//
		$spb_detail_sql =  "SELECT * FROM `as_detail_spb` WHERE spbNo = '$spbNoX'";
		//print_r($spb_detail_sql);
		// looping detail buku
		$spb_detail = $conn->query($spb_detail_sql);
		$total_spb = $spb_detail->num_rows;

		// bnyaknya produk yg dibeli dalam invoice
		$qty_produk = $spb_detail->num_rows;
		//echo $qty_produk;
		//$total_
		//return ;
		//echo "=====================================================";
		$total_trx = 0;
		$total_untung = 0;
		// di looping buat hitung total laba
		while ($data = $spb_detail->fetch_assoc()) {
			// detail_produk
			$productID = $data['productID'];
			$sql_produk = "SELECT * FROM as_products WHERE productID = '$productID'";
			$data_produk = $conn->query($sql_produk)->fetch_assoc();

			// dapetin harganya
			$harga_beli_produk = $data_produk['purchasePrice'];
			$harga_asli_produk = $data_produk['realPrice'];
			//
			$total_untung += ($harga_beli_produk - $harga_asli_produk)* $data['qty'];

			//print_r($data_produk);
			// /print_r($data);
		}
		$tgl_laba = date("Y-m-d");
		$data_laba = ['invoiceID' => $invoiceID,
					'qtyProduk' => $qty_produk,
					'nilaiLaba' => $total_untung,
					'tglLaba' => $tgl_laba];
		// simpan data laba ke tabel
		$sqlLaba = "INSERT INTO as_laba (	invoiceID, 
											qtyProduct,
											nilaiLaba,
											tglLaba
										) VALUES (
        ...
        // ekseksi
		$queryLaba = $conn->query($sqlLaba);
		print_r(json_encode($data_laba));
