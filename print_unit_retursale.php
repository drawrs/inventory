...
if ($_SESSION['staffID'] == '' && $_SESSION['email'] == '')
{
	...
	if ($module == 'retursale' && $act == 'print')
	{
		...
		$filename="retur_penjualan.pdf";
		$content = ob_get_clean();
		
		// showing up the main retur data
		$queryMain = "SELECT * FROM as_retur_staffs WHERE returID = '$returID' AND returNo = '$returNo'";
		$sqlMain = mysqli_query($connect, $queryMain);
		$dataMain = mysqli_fetch_array($sqlMain);
		
		$returDate = tgl_indo2($dataMain['returDate']);
		...
		if ($dataMain['returType'] == '1')
		{
			$returType = "Cash";
		}
		elseif ($dataMain['returType'] == '2')
		{
			$returType = "Saldo";
		}
		else
		...
		$content = "<table style='padding-bottom: 10px; width: 240mm;'>
						...
						<tr valign='top'>
							...
							<td>
								<span style='font-size: 11pt;'><b>BUKTI RETUR PENJUALAN</b></span>
							</td>
						</tr>
					</table>
					<table cellpadding='0' cellspacing='0' style='width: 240mm;'>
						<tr>
							...
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt; width: 28mm;'>No Faktur Jual</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt; width: 3mm;'>:</td>
						...
						<tr>
							...
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$returDate</td>
						...
						<tr valign='top'>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>Cara Retur</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>:</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$returType</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'></td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'></td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'></td>
					...
					<table cellpadding='0' cellspacing='0' style='width: 240mm; border-bottom: 1px solid #000;'>
						...
						$queryReturDetail = "SELECT * FROM as_detail_retur_staffs WHERE returNo = '$returNo' ORDER BY detailID ASC";
						...
						while ($dtReturDetail = mysqli_fetch_array($sqlReturDetail))
						{
							...
							$unitPrice = rupiah($dtReturDetail['unitPrice']);
							
							$content .= "
									<tr valign='top'>
										<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$i</td>
										<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$productName</td>
										<td style='padding: 2px 30px 2px 0px; font-size: 9pt; text-align: right;'>$unitPrice</td>
					...
					<table cellpadding='0' cellspacing='0' style='width: 230mm;'>
						<tr valign='top'>
							...
							<td style='padding: 5px 0px 5px 0px; font-size: 9pt; text-align: right; width: 100mm;'>
								<table>
									<tr>
										<td style='width: 60mm; text-align: right;'>Jumlah Harga Retur</td>
										<td style='width: 5mm;'>:</td>
										<td style='text-align: right; width: 35mm;'>$subtotal</td>
									</tr>
									<tr>
