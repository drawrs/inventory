...
else 
{
	...
	elseif ($module == 'retursale' && $act == 'input')
	{
		...
		if ($sqlRetur)
		{
			...
			$queryUpdate = "UPDATE as_retur_staffs SET subtotal = '$subtotal', ppn = '$ppn', grandtotal = '$grandtotal' WHERE returID = '$returID'";
	...
	elseif ($module == 'retursale' && $act == 'add')
	{
		...
		// show the bbm data
		$queryBSale = "SELECT * FROM as_sales_transactions WHERE invoiceNo = '$invoiceNo'";
		...
		$queryDoDetail = "SELECT * FROM as_detail_do WHERE doNo = '$dataBSale[doNo]' ORDER BY doID ASC";
	...
	elseif ($module == 'retursale' && $act == 'search')
	{
		...
		if ($sDate != '' || $eDate != '')
		{
			$queryRetur = "SELECT * FROM as_retur_staffs WHERE returNo LIKE '%$q%' AND returDate BETWEEN '$startDate' AND '$endDate' ORDER BY returNo DESC";
		}
		else
		{
			$queryRetur = "SELECT * FROM as_retur_staffs WHERE returNo LIKE '%$q%' ORDER BY returNo DESC";	
		}
		$sqlRetur = mysqli_query($connect, $queryRetur);
