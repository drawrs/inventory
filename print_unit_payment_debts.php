...
if ($_SESSION['staffID'] == '' && $_SESSION['email'] == '')
{
	...
	if ($module == 'payment' && $act == 'print')
	{
		...
		$filename="rincian_pembayaran_hutang.pdf";
		$content = ob_get_clean();
		
		// showing up the main debt data
		$queryMain = "SELECT * FROM as_debts WHERE debtID = '$debtID' AND debtNo = '$debtNo'";
		$sqlMain = mysqli_query($connect, $queryMain);
		$dataMain = mysqli_fetch_array($sqlMain);
		
		// showing up the payment data
		...
		$debtTotal = rupiah($dataMain['debtTotal']);
		$incomingTotal = rupiah($dataMain['incomingTotal']);
		$reductionTotal = rupiah($dataMain['reductionTotal']);
		$s = $dataMain['debtTotal'] - ($dataMain['incomingTotal'] + $dataMain['reductionTotal']);
		...
		$content = "<table style='padding-bottom: 10px; width: 240mm;'>
					...
					<table cellpadding='0' cellspacing='0' style='width: 240mm;'>
						<tr>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt; width: 28mm;'>Nomor Bukti</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt; width: 3mm;'>:</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt; width: 35mm;'>$dataPay[paymentNo]</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt; width: 28mm;'>Terhutang</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt; width: 3mm;'>:</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt; width: 35mm;'>$debtTotal</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt; width: 28mm;'>Supplier :</td>
							<td colspan='2'></td>
						</tr>
						<tr>
						...
						<tr valign='top'>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>Valas</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>:</td>
							<td style='padding: 2px 0px 2px 0px; font-size: 9pt;'>$dataMain[valas]</td>
					...
					<table cellpadding='0' cellspacing='0' style='width: 240mm; border-bottom: 1px solid #000;'>
						...
						$queryPayment = "SELECT * FROM as_buy_payments WHERE invoiceNo = '$dataMain[invoiceNo]' AND invoiceID = '$dataMain[invoiceID]' ORDER BY paymentDate ASC";
