...
else 
{
	...
	if ($module == 'bbm' && $act == 'delete')
	{
		...
		{
			$querySP = "UPDATE as_stock_products SET stock=stock-$dataBD[receiveQty] WHERE productID = '$dataBD[productID]' AND factoryID = '$dataBD[factoryID]'";
	...
	elseif ($module == 'bbm' && $act == 'input')
	{
		...
		$dataSpb = mysqli_fetch_array(mysqli_query($connect, "SELECT spbID FROM as_spb WHERE spbNo = '$spbNo'"));
		...
		for ($i = 0; $i < $countDetailID; $i++)
		{
			...
			$queryStock = "UPDATE as_stock_products SET stock=stock+$receiveQty[$i] WHERE factoryID = '$factoryID' AND productID = '$productID[$i]'";
	...
	elseif ($module == 'bbm' && $act == 'add')
	{
		...
		// show the bbm data
		$queryBbm = "SELECT A.bbmID FROM as_bbm A INNER JOIN as_detail_bbm B ON B.bbmNo=A.bbmNo WHERE A.spbNo = '$spbNo'";
		$sqlBbm = mysqli_query($connect, $queryBbm);
		$numsBbm = mysqli_num_rows($sqlBbm);
		...
		$i = 1;
		$querySpbDetail = "SELECT * FROM as_detail_spb WHERE spbNo = '$spbNo' AND spbFaktur = '$dataSpb[spbFaktur]'";
